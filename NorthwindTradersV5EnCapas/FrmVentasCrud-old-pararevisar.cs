            ...
            else if (tabcOperacion.SelectedTab == tabpModificar)
            {
                try
                {
                    if (ValidarControlesVenta())
                    {
                        ...
                        {
                            U.NotificacionWarning(strVenta + " ha sido modificada por otro usuario de la red, no se realizará la actualización del registro, vuelva a cargar el registro para que se muestre la venta con los datos proporcionados por el otro usuario");
                            return;
                        }
                        else if (result == -4)
                        {
                        ...
                        venta.RowVersion = (txtId.Tag != null && long.TryParse(txtId.Tag.ToString(), out long tagVal))
